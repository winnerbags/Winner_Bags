...
def next_invoice_no(inv_type: str, fy: str) -> str:
    ...
    if inv_type == "GST":
        pfx = "RU" if is_rb else "WB01"
        rgx = r"^RU\d" if is_rb else r"^WB01\d"
        skip = 2 if is_rb else 4
        pipeline = [
            {"$match": {
                "InvoiceNo": {"$regex": rgx},
        ...
        pfx2 = "RUN" if is_rb else "WB01N"
        rgx2 = r"^RUN" if is_rb else r"^WB01N"
        skip2 = 3 if is_rb else 5
        pipeline = [
            {"$match": {
...
def next_quotation_no(q_type: str, fy: str) -> str:
    ...
    if q_type == "GST":
        pipeline = [
            {"$match": {"QuotationNo": {"$regex": r"^WB_Q01\d"}, "QuotationDate": {"$gte": fy_from, "$lte": fy_to}}},
            {"$project": {"num": {"$toInt": {"$substr": ["$QuotationNo", 6, 10]}}}},
            {"$group": {"_id": None, "max": {"$max": "$num"}}}
        ]
        res = list(col("quotations").aggregate(pipeline))
        n = (res[0]["max"] if res else 0) + 1
        return f"WB_Q01{n:02d}"
    else:
        pipeline = [
            {"$match": {"QuotationNo": {"$regex": r"^WB_Q01N"}, "QuotationDate": {"$gte": fy_from, "$lte": fy_to}}},
            {"$project": {"num": {"$toInt": {"$substr": ["$QuotationNo", 7, 10]}}}},
            {"$group": {"_id": None, "max": {"$max": "$num"}}}
        ]
        res = list(col("quotations").aggregate(pipeline))
        n = (res[0]["max"] if res else 0) + 1
        return f"WB_Q01N{n:02d}"

def next_product_code() -> str:
    pipeline = [
        {"$match": {"Code": {"$regex": r"^WB_P01"}}},
        {"$project": {"num": {"$toInt": {"$substr": ["$Code", 6, 10]}}}},
        {"$group": {"_id": None, "max": {"$max": "$num"}}}
    ]
    res = list(col("products").aggregate(pipeline))
    n = (res[0]["max"] if res else 0) + 1
    return f"WB_P01{n:03d}"
...
async def billing_delete_invoice(inv_id: int):
    ...
    # Determine prefix family: RUN/RN (non-GST) or RU/R (GST)
    if inv_no.startswith("RUN"):
        regex = r"^RUN"
...
async def billing_delete_quotation(quot_id: int):
    ...
    if qno.startswith("QN"):
        latest = col("quotations").find_one({"QuotationNo": {"$regex": r"^WB_Q01N"}}, sort=[("Id", DESCENDING)])
    else:
        latest = col("quotations").find_one({"QuotationNo": {"$regex": r"^WB_Q01\d"}}, sort=[("Id", DESCENDING)])
...
async def update_user(username: str, request: Request):
    ...
    col("winner_bags_users").update_one({"username": username}, {"$set": update})
    return ok()
...
async def delete_user(username: str):
    ...
    col("winner_bags_users").delete_one({"username": username})
    return ok()
